...
import io.github.debop.connector.BackendCConnector.Companion.CONNECTOR_C_NAME
 ...
 */
@Retry(name = CONNECTOR_C_NAME)
@CircuitBreaker(name = CONNECTOR_C_NAME)
@RateLimiter(name = CONNECTOR_C_NAME)
@Component(value = CONNECTOR_C_NAME + "Connector")
class BackendCConnector : Connector {

    companion object {
        const val CONNECTOR_C_NAME = "backendC"
    }

    @Bulkhead(name = CONNECTOR_C_NAME)
    ...
    override fun ignoreException(): String {
        throw BusinessException("This exception is ignored by the CircuitBreaker of $CONNECTOR_C_NAME")
    }
    ...
    @CircuitBreaker(name = CONNECTOR_C_NAME, fallbackMethod = "fallback")
    override fun failureWithFallback(): String {
        return failure()
    }

    @Bulkhead(name = CONNECTOR_C_NAME, type = Bulkhead.Type.THREADPOOL)
